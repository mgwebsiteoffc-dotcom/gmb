...
@section('content')
<form method="POST" action="{{ route('admin.clients.store') }}" enctype="multipart/form-data" class="max-w-2xl">
    @csrf
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <div id="logo-preview" class="mt-2 w-14 h-14 rounded-2xl flex items-center justify-center text-3xl bg-slate-100 border border-slate-200">{{ old('logo', '🏢') }}</div>
            </div>
            <div class="space-y-4">
                <div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input name="mobile" value="{{ old('mobile') }}" placeholder="+91 98xxxxxxx0" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input name="onboarded_at" type="date" value="{{ old('onboarded_at') }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input name="poc_name" value="{{ old('poc_name') }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
                </div>
                <div>
                    <label class="block text-[11px] font-bold text-slate-500 mb-1">POC Email</label>
                    <input name="poc_email" type="email" value="{{ old('poc_email') }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
                </div>
                <div>
                    <label class="block text-[11px] font-bold text-slate-500 mb-1">POC Mobile</label>
                    <input name="poc_mobile" value="{{ old('poc_mobile') }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
                </div>
            </div>
        </div>
